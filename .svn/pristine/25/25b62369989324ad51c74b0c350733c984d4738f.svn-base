...
{
	[self removeCoverViewInSelf:YES];
}
...
- (void)removeCoverViewInSelf:(BOOL)displayDeleFalge
{
...
- (void)addGridItem:(CustomGridButton *)customItem
{
	...
	//更新页面
	[self getNewData];
	[[MTSingletonManager shareInstance] syncMutableArrayData:_gridListArray girdType:MTSingleDisplayArrayType];
	if (flag) {
		[self createViewToCover];
	}
}
