...
{
	if (self.realWebView) {
		[_realWebView JSE_enableEngine];
	}
}

- (void)JSE_registerNative:(NSString*)handlerName handle:(id)handler
{
	if (self.realWebView) {
		[_realWebView JSE_registerNative:handlerName handle:handler];
	}
}

- (void)JSE_registerNative:(NSString *)handlerName withAsyncHandle:(id)handler
{
	if (self.realWebView) {
		[_realWebView JSE_registerNative:handlerName withAsyncHandle:handler];
	}
}

- (void)JSE_executeJS:(NSString*)handlerName data:(id)data callback:(id)callback
{
	if (self.realWebView) {
		[_realWebView JSE_executeJS:handlerName data:data callback:callback];
	}
}

- (void)JSE_setWebViewDelegate:(id)delegate
{
	if (self.realWebView) {
		[_realWebView setWebViewDelegate:delegate];
	}
}
